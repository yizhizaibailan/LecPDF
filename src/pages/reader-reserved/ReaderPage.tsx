 ...
 */
import type { DocumentSource, ReaderEvent, ReaderSession } from '../../types/reader'
...
export type ReaderPageProps = {
  ...
  /** 将数据层阅读事件回写当前会话；页面不接触 Store 或阅读内核。 */
 ...
 */
export function ReaderPage({ session, source, onReaderEvent }: ReaderPageProps): JSX.Element {
  ...
  if (session.kind === 'pdf' && source?.kind === 'pdf') return <PdfReaderPage key={session.tabId} url={source.url} onReaderEvent={onReaderEvent} />
